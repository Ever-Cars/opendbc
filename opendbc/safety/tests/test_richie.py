...
class TestRichie(TestDefaultRxHookBase):
  TX_MSGS = [[addr, bus] for addr in [GM_CAMERA_DIAG_ADDR, *range(0x600, 0x800),
                                      *range(0x18DA00F1, 0x18DB00F1, 0x100),  # 29-bit UDS physical addressing
                                      *range(0x17FC0076, 0x17FD0076, 0x100),  # 29-bit UDS physical addressing
                                      *range(0x1DD00001, 0x1DD10001, 0x100),  # 29-bit UDS physical addressing
  ...
  def setUp(self):
    ...
    self.safety.set_safety_hooks(CarParams.SafetyModel.richie, 0)
  ...
  def test_tx_hook(self):
    ...
    # Richie only allows 8 byte UDS/KWP messages right now
